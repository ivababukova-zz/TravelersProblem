...
% gomory's fundamental work on mixed integer linear programming
...
\textcolor{red}{
We model TP as an integer and as a constraints programming problem. The constraint programming approach is explained in Section \ref{subsec:tpcp} and the integer programming model is presented in Section \ref{subsec:tpip}.
}
...
\label{subsec:tpcp}
\textcolor{red}{
Let $m$ be equal to $|F|$. We introduce an array $\mathcal{S}$ of size $m$ that represents the TP tour. The domain of each variable in $\mathcal{S}$ is $\{0,...,m\}$. If $\mathcal{S}[i] = j$, then flight $f_{j} \in F$ is the i$^{th}$ flight in the tour. To denote the end of the tour at some position $z + 1< m$, we set $\mathcal{S}[z+1] = 0$. All subsequent variables in $\mathcal{S}$ will then have to be 0 and we say that the tour is of size $z$.
}
...
\color{red}
\sum^{m}_{j = 1} c_{k} k, \quad \textrm{where } k=\mathcal{S}[j]
\end{equation}

\textcolor{red}{subject to the following constraints:}
...
\Item
$\textrm{allDiff}(\mathcal{S}[0],...,\mathcal{S}[z]), \quad \forall \, z, \, 0 \leq z < m \textrm{ s.t } \mathcal{S}[z] \neq 0$
\end{itemize}

\textcolor{red}{Constraint (10) restricts that once the end of the tour is reached at some position $j$, no flights are further added to the tour. Constraint (11) enforces that every flight in the tour is taken only once, using an all-different constraint \citep{Hoeve01}.}

\textcolor{red}{The five properties of a valid tour are added to the model as the following constraints:}
\begin{itemize}
\color{red}
% property 1:
\Item
$\textrm{d}_{\mathcal{S}[0]} = \{j \in \{1,...,m\} : A^{d}_{j} = A_{0}\}$
\item[]
$(\mathcal{S}[z] \neq 0 \wedge \mathcal{S}[z+1] = 0) \, \Rightarrow \,
(\textrm{d}_{\mathcal{S}[z]} = \{j \in \{1,...,m\} : A^{a}_{j} = A_{0}\})$
% property 2:
\Item
$\forall \, i>1, \textrm{ d}_{\mathcal{S}[i]} = \{0, j \in \{1,...,m\} : A^{d}_{j} = A^{a}_{p}, \, p = \mathcal{S}[i-1]\}$
% property 3:
\Item
$t_{p} + \Delta_{p} + C_{r} \leq t_{q}, \textrm{ where } p = \mathcal{S}[j], \, q = \mathcal{S}[j+1],\, r = A^{a}_{q}, \quad \forall \, j, \,  0 \leq j < m - 1$
% property 4:
\Item
$(\mathcal{S}[z] \neq 0 \wedge \mathcal{S}[z+1] = 0) \, \Rightarrow \, (t_{q} + \Delta_{q} \leq T), \textrm{ where } q = \mathcal{S}[z]$
% property 5:
\Item
$\forall A_{k} \in A, \, J_{k} = \{ j \in \mathcal{S}[i] \, | \, A^{a}_{j} = A_{k} \textrm{ for } 0 \leq i < m\}. \textrm{ } \forall k, \, A_{k} \in D \, \Rightarrow \, |J_{k}| > 0$
\end{itemize}

\textcolor{red}{Constraint (12) is equivalent to property (1). Each of the two equations restricts the domain of the corresponding variable in $\mathcal{S}$ to contain only flights that depart from/arrive at the home point. Property (2) is enforced by constraint (13). The domain of each variable in $\mathcal{S}$ is set to include only flights that depart from the arrival airport of the previous flight. Constraints (14) and (15) did not undergo substantial changes from properties (3) and (4). To enforce that every destination airport is visited at least once (constraint (16)), we create a set $J$, where $J_{k}$ is equal to the number of flights that arrive at $A_{k}$ in the tour. If $A_{k}$ is a destination, then the size of $J_{k}$ can only be positive.}

\subsection{IP Model}
\label{subsec:tpip}
\textcolor{red}{Most of the constraints for the TP CP model need modification in order to be applicable to a IP model. In particular, IP does not allow for ``if-then'', ``all-different'' and other non-integer constraints. The model described in this section is a modification of the TP CP model that complies with the rules of integer programming.}

\textcolor{red}{Let $m$ be equal to $|F|$. We introduce a variable $x_{i,j}$ for every $i,j$  between 0 and $m$, such that $x_{i,j} = 1$ if $i^{th}$ flight is $f_{j}$ or 0 otherwise. This variable is somewhat similar to $\mathcal{S}$, used for the CP model, where $\mathcal{S}[i] = p$ is equivalent to $x_{i,p} = 1$. The objective function of TP is to minimise:}

\begin{equation}
\label{eq:TPwithIPobj}
\color{red}
\sum_{j=1}^{m} \sum_{i=1}^{m} c_{j} x_{i,j}
\end{equation}
...
\color{red}
\Item
$\forall \, i, \, \sum^{m}_{j = 1}x_{i,j} \leq 1 $
\Item
$\forall \, j, \, \sum^{m}_{i = 1}x_{i,j} \leq 1 $
\end{itemize}

\textcolor{red}{Constraint (18) restricts that only one flight is taken at each step $i$. Constraint (19) is equivalent to the all-different constraint (11): it enforces every flight to be taken at most once.}

\textcolor{red}{The five properties of  valid tour are expressed as follows:}

\begin{itemize}
\color{red}
% property 1:
\Item
$\sum_{j \in S_{1}} x_{1,j} = 1, \quad \textrm{where } S_{1} = \{j \in \{1,...,m\} : A^{d}_{j} = A_{0}\}$
\Item
$\sum_{j \in S_{2}} x_{z,j} = 1 \wedge \sum_{j=1}^{m} x_{z+1,j} = 0, \quad \textrm{where } S_{2} = \{j \in \{1,...,m\} : A^{a}_{j} = A_{0}\}$
\end{itemize}

\textcolor{red}{Constraint (20) enforces that the flight taken at step 1 must depart from the home point. Constraint (21) enforces an end to the sequence for some integer $z$ and makes the last flight arrive at the home point. Property (1) of a valid tour is expressed by enforcing both (20) and (21).}

\begin{itemize}
\color{red}
% property 2:
\Item
$\sum_{j \in S_{1}} x_{i-1,j} = \sum_{j^{\prime} \in S_{2}} x_{i,j^{\prime}}, \quad \forall 1<i\leq z \wedge \forall y \in A, \textrm{ where}$
\item[]
$S_{1} = \{j \in \{1,...,m\} : A^{a}_{j} = y\} \textrm{ and } S_{2} = \{j^{\prime} \in \{1,...,m\} : A^{d}_{j^{\prime}} = y\}$
\end{itemize}

\textcolor{red}{To add property (2) to the IP model, we define the sets of flights $S_{1}$ and $S_{2}$, such that all flights in $S_{2}$ depart from the arrival airport of $S_{1}$. Constraint (22) restricts that every flight always departs from the arrival airport of the previous taken flight, assuming that is it non-zero, where $z$ denotes the number of flights in a tour.}

\begin{itemize}
\color{red}
% property 3:
\Item
$\sum_{j = 1}^{m} x_{i-1,j} = \sum_{j^{\prime} \in S} x_{i,j^{\prime}}, \quad \textrm{where } S= \{j^{\prime} \in \{1,...,m\} : t_{j} + \Delta_{j} + C_{r} \leq t_{j^{\prime}}, \, r = A^{d}_{j}\}$
% property 4:
\Item
$\sum_{j \in S} x_{z,j} = 1, \quad \textrm{where } S = \{j \in \{1,...,m\} : t_{j} + \Delta_{j} \leq T\}$
\end{itemize}

\textcolor{red}{Properties (3) and (4) are expressed similarly to (2) by defining the set of flights which the $j$ of every $x_{i,j}$ can be chosen from. The properties are enforced via constraints (23) and (24) respectively.}

% property 5:
\textcolor{red}{To enforce property (5), we introduce a new variable $\mathcal{D}_{i,j}$. Let $k = |D|$, that is the number of destinations in the TP instance. For every $i$, $\mathcal{D}_{i,j} = 1$ if there is a flight $f_{j}$ in the tour such that $A^{a}_{j} = D_{i}$. Since each destination has to be visited in the tour, the sum of all $i$, there must be a $j$ such that $\mathcal{D}_{i,j} = 1$. This can be expressed as the following IP constraint:}

\begin{itemize}
\color{red}
\Item
$\forall \, i, \, \sum_{j \in J}\mathcal{D}_{i,j} \geq 1,\quad J = \{j \in \{1,...,m\} : x_{i,j}=1 \wedge A^{a}_{j} = D_{i} \}$
\end{itemize}
