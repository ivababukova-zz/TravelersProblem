...
\item An array $f$ of $m$, where $m = |F|$. For each $0 < i < m$, $f[i]$ is 1 if flight $F[i]$ is taken in a TP tour, or 0 otherwise.
%variables that represents whether a flight is taken in a solution to TP or not. For each $0 \leq i < k$, $f[i]$ = 1 if i-th flight in $F$ is in the solution sequence, or 0 otherwise.

\item an array $s$ of size $k$, which is the sequence of flights taken during the trip. Each variable in $s$ has domain ${0, k}$. If $s[i]$ is 0, then the trip is finished. Therefore, the value of every subsequent variable in $s$ will also be 0. If no variables in $s$ are equal to 0, then the solution is a trip consisting of all flights.

\item an array $c$ of size $n$ which represents the number of times each city is visited during the trip. The value of each variable must be even.
\end{itemize}
