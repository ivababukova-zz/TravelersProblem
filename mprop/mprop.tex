...
Let $m$ be equal to $|F|$. We introduce an array $\mathcal{S}$ of size $m$ that represents the TP tour. The domain of each variable in $\mathcal{S}$ is $\{0,...,m\}$. If $\mathcal{S}[i] = j$, then flight $f_{j} \in F$ is the i$^{th}$ flight in the tour. To denote the end of the tour at some position $z < m$, we set $\mathcal{S}[z+1] = 0$. All subsequent variables in $\mathcal{S}$ will then have to be 0 and we say that the tour is of size $z$.
...
\Item
$\textrm{allDiff}(\mathcal{S}[0],...,\mathcal{S}[z]), \quad \forall \, z, \, 0 \leq z < m \textrm{ s.t } \mathcal{S}[z] \neq 0 \wedge \mathcal{S}[z+1] = 0$
\end{itemize}

Constraint (10) restricts that once the end of the tour is reached at some position $j$, no flights are further added to the tour. Constraint (11) enforces that every flight in the tour is taken only once.

The constraints equivalent to the five properties of a valid tour:
\begin{itemize}
% property 1:
\Item
$\textrm{dom}_{\mathcal{S}[0]} = \{j \in \{1,...,m\} : A^{d}_{j} = A_{0}\}$
\item[]
$(\mathcal{S}[z] \neq 0 \wedge \mathcal{S}[z+1] = 0) \, \Rightarrow \,
(\textrm{dom}_{\mathcal{S}[z]} = \{j \in \{1,...,m\} : A^{a}_{j} = A_{0}\})$
% property 2:
\Item
$\forall \, i>1, \textrm{ dom}_{\mathcal{S}[i]} = \{0, j \in \{1,...,m\} : A^{d}_{j} = A^{a}_{p}, \, p = \mathcal{S}[i-1]\}$
% property 3:
\Item
...
% property 5:
\Item
$\textrm{Let } J_{k} = \{ j \in \mathcal{S}[i] \, | \, A^{a}_{j} = A_{k} \textrm{ for } 0 \leq i < m\}. \textrm{ } \forall k, \, A_{k} \, \Rightarrow \, |J_{k}| > 0$
\end{itemize}

Constraints 12-16 enforce any assignment of variables to $\mathcal{S}$ to be a valid tour. To restrict the first and the last flight to depart and arrive at the home point respectively, we restrict the domain of the corresponding variable in $\mathcal{S}$ to contain only flights that depart from/arrive at the home point.

\subsection{IP Model}
Let $m$ be equal to $|F|$. We introduce a variable $x_{i,j}$ for every $i,j$  between 0 and $m$, such that $x_{i,j} = 1$ if $i^{th}$ flight is $f_{j}$ or 0 otherwise. A TP tour is then the sequence of flights $\mathcal{S} = \{f_{p},...,f_{q}\}$, where $\mathcal{S}[i] = p$ iff $x_{i,p} = 1$ and $|\mathcal{S}| = \sum_{j=1}^{m} \sum_{i=1}^{m} x_{i,j}$. The objective function of TP is then to minimise:
...
\begin{itemize}
\Item
$\forall \, i, \, \sum^{m}_{j = 1}x_{i,j} \leq 1,\quad \textrm{(take at most one flight at each step }i) $
\Item
$\forall \, j, \, \sum^{m}_{i = 1}x_{i,j} \leq 1,\quad \textrm{(take flight }f_{j} \textrm{ at most once)} $
\end{itemize}
...
To add property (2) to the IP model, we define the sets of flights $S_{1}$ and $S_{2}$, such that all flights in $S_{2}$ depart from the arrival airport of $S_{1}$. Constraint (22) restricts that every flight always departs from the arrival airport of the previous taken flight, assuming that is it non-zero, where $z$ denotes the number of flights in a tour.
...
% property 5:
To enforce property (5), we introduce a new variable $\mathcal{D}_{i,j}$. Let $k = |D|$, that is the number of destinations in the TP instance. For every $i$, $\mathcal{D}_{i,j} = 1$ if there is a flight $f_{j}$ in the tour such that $A^{a}_{j} = D_{i}$. Since each destination has to be visited in the tour, the sum of all $i$, there must be a $j$ such that $\mathcal{D}_{i,j} = 1$. This can be expressed as the following IP constraint:
