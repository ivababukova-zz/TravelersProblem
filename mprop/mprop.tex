...
% todo: find out more about Concorde and summarise it.
\textcolor{red}{
...
%% todo: what does this dual model consist of?
...
\begin{itemize}
\Item
$t_{i} - t_{1} \geq d(1,i), \quad (2 \leq i \leq n )$
\Item
$|t_{i} - t_{j}| \geq d(i,j), \quad (2 \leq j < i)$
\Item
$t_{n+1} - t_{i} \geq d_{i,1}, \quad (2 \leq i \leq n )$
\Item
$t_{i} \geq 0, \quad (1 \leq i \leq n )$
\Item
$l_{i} \leq t_{i} \leq u_{i}, \quad (2 \leq i \leq n )$
\end{itemize}
...
\citet{Hurkala15} is a recent work that is believed to be the first to consider problems with multiple time windows. The paper agrees with \citet{ariglianotime} that the constant travel time assumption is unrealistic and the TCTSP model is also adapted to changing travel times. \citet{Hurkala15} introduces a novel algorithm for this re-formulation of TCTSP and combines it with three already existing metaheuristics to produce three TCTSP solvers. \textcolor{red}{For the empirical evaluation, \citet{Hurkala15} uses 45 randomly generated instances with 13 to 23 cities, which is almost twice smaller than the maximum number of cities that \citet{ariglianotime} can solve. This suggests that multiple time windows add additional complexity to the problem.} %The algorithms produce similar results. However, for some instances one algorithm produces better results, while for others it performs worse.}
...
\textcolor{red}{
Having two distinct models would enable us to compare the effectiveness of the used algorithms. Moreover, any conclusions on the problem will be less dependent on the chosen model.}
...
Constraint (18) restricts that once the end of the trip is reached at some position $z$, no flights are further added to $\mathcal{S}$. Constraint (19) enforces that every flight is taken only once, using an all-different constraint \citep{Hoeve01}.
...
Constraint (20) is equivalent to trip property (1). It restricts the domains of the first and the $(z-1)^{th}$ variable in $\mathcal{S}$ to contain only flights that depart from/arrive at the home point. Trip property (2) is enforced by constraint (21). The domain of each variable in $\mathcal{S}$ is set to include only flights that depart from the arrival airport of the previous flight. Constraints (22) and (23) correspond to trip properties (3) and (4) respectively. Constraint (24) restricts that the number of the flights that arrive at every destination in the trip is positive, and thus enforces trip property (5).
...
subject to constraints (26-32).

First, we restrict that only one flight is taken at each step $i$ with constraint (26). Constraint (27) is equivalent to the all-different constraint (14): it enforces every flight to be taken at most once.
...
Constraints (28) and (29) enforce trip property (1). The first flight is restricted to depart from the home point and the last flight must arrive at the home point.
From trip property (2) it follows that there must exist some $z < m$, $x_{z,j} = 1$, for which $j \neq 0$ and $A^{a}_{j} = A_{0}$. Thus, constraint (29) indirectly enforces that $A^{a}_{j} = A_{0}$.
...
Property (2) is enforced by defining two sets of flights $S_{1}$ and $S_{2}$, such that all flights in $S_{2}$ depart from the arrival airport of $S_{1}$. Constraint (30) restricts that every flight always departs from the arrival airport of the previous taken flight.
...
Give an example of how would you model one constraint.

\subsection{Choice of CP and IP solvers}
\label{sec:cpandisolvers}
\textcolor{red}{
We were unable to find a solver which supports both IP and CP and this is the reason why the TP IP and CP models will be implemented using two different solvers. In order to decrease the implementation difference between the two models as much as possible, we have chosen solvers that support the same language, that is Java.}
...
\section{Work Plan}
\label{sec:workplan}
%todo: maybe make a section about project requirements and decide which are of most priority
